...
def create_acm_mapping(df: pd.DataFrame, barcode: str, base_acm_path: str) -> pd.DataFrame:
...
def main():
    ...
    parser.add_argument('--base_acm_path', required=True, help="Base path for merged ACM files (e.g., /all_cells_mask/{barcode}/infer_{barcode}_multiscale)")
    args = parser.parse_args()

    # Step 1: Load and map ACM paths
    df = load_input_csv(args.barcode)
    mapped_df = create_acm_mapping(df, args.barcode, args.base_acm_path)
