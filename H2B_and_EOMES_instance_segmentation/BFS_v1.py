...
# -------------------- Shutdown Button --------------------
st.subheader(
    "Click this button to shutdown streamlit GUI server so the process closes properly"
)
...
def load_image(image_path):
    ...
    print(
        f"[File Loader] Loaded file {image_path} with shape {bio_img.shape} in {time.time()-startload:.2f}s"
    )
    try:
...
def RescaleInputImage(image_offset_corrected, val1, val2, constant_mode=False):
    ...
    if constant_mode:
        ...
        scale = (
            1.0 if maximum_val == minimum_val else 65535.0 / (maximum_val - minimum_val)
        )
...
def process_timepoint(
    ...
    start = time.time()
    if constant_mode:
        image_rescaled, scale = RescaleInputImage(
            image, raw_max_val, p99_const, constant_mode=True
        )
    else:
        image_rescaled, scale = RescaleInputImage(
            image, image.min(), image.max(), constant_mode=False
        )
    ...
    input_scale_log.append(
        [f"{image_name}_T{t}", scale, image.min(), image.max(), p1, p99]
    )
    local_eval_params = eval_params.copy()
    local_eval_params["lowhigh"] = [p1, p99]

    start = time.time()
    denoised_image = denoise_model.eval(
        x=image_rescaled, channels=[0, 0], tile=False, normalize=local_eval_params
    )
    ...
    output_scale_log.append(
        [f"{image_name}_T{t}", scale, output_raw_min, output_raw_max, p1_out, p99_out]
    )
    ...
    if timepoint_range and timepoint_range.strip().lower() != "all":
        try:
            ...
            end_tp = (
                ...
                else num_timepoints - 1
            )
        except Exception as e:
            print(
                f"Error parsing timepoint range '{timepoint_range}': {e}. Processing all timepoints."
            )
...
def run_denoising(
    ...
    with ThreadPoolExecutor(max_workers=file_workers) as file_executor:
        ...
        for future in tqdm(
            as_completed(file_futures), total=len(file_futures), desc="Loading files"
        ):
            try:
                tasks = future.result()
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as task_executor:
        ...
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Processing timepoints"
        ):
            try:
...
def process_image_constant_timepoint(
    bio_img,
    t,
    image_name,
    denoise_model,
    ...
    image_rescaled, scale = RescaleInputImage(
        image_offset_corrected, raw_max_val, p99, constant_mode=True
    )
    local_eval_params = eval_params.copy()
...
def constant_denoise_directory(
    input_dir,
    output_dir,
    model_params,
    eval_params,
    ...
    for image_path in image_paths:
        ...
        if timepoint_range and timepoint_range.strip().lower() != "all":
            try:
                ...
                start_tp = int(parts[0]) - 1
                end_tp = (
                    int(parts[1]) - 1 if len(parts) > 1 and parts[1] != "" else T - 1
                )
            except Exception as e:
                start_tp, end_tp = 0, T - 1
        else:
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as executor:
        futures = [
            executor.submit(process_image_constant_timepoint, *task) for task in tasks
        ]
        results = []
    ...
    for image_name, t, img_path in results:
        tp_name = f'{image_name}_T{t:04d}'
        denoised_dict[tp_name] = (img_path, t)
    return denoised_dict
...
def calculate_region_props(mask_img, raw_img, properties, filename):
    try:
        ...
        props = regionprops_table(
            mask_img_np, intensity_image=raw_img_np, properties=properties
        )
...
def fill_holes_3d_gpu(mask, min_hole_diameter=64.0):
    ...
    for z in range(mask.shape[0]):
        ...
        for label in slice_labels:
            ...
            filled_binary = skimage.morphology.remove_small_holes(
                binary_mask, area_threshold=min_hole_size
            )
...
def process_single_tif(
    ...
    try:
        ...
        print(
            f"File {mask_filename}: {len(labels)} labels before pre-clipping filtering."
        )
        # print(f"Areas: {cp.asnumpy(areas)}")
        # print(f"Mean Intensities: {cp.asnumpy(mean_intensities)}")

        valid_indices = (areas >= pre_clipping_min_size) & (
            mean_intensities >= pre_clipping_min_mean_intensity
        )
        filtered_labels = labels[valid_indices]
        print(
            f"File {mask_filename}: {len(filtered_labels)} labels after pre-clipping filtering."
        )
        ...
        for label in filtered_labels:
            ...
            if cp.any(label_mask):
                ...
                modified_mask = adaptive_clip(
                    raw_img,
                    modified_mask,
                    label,
                    computed_mean_intensity,
                    scaling_factor,
                    slice_scaling_percentile,
                    mask_filename,
                )
        ...
        props_post = calculate_region_props(
            modified_mask, raw_img, properties, mask_filename
        )
        ...
        valid_post_indices = (post_areas >= post_clipping_min_size) & (
            post_integrated_intensity >= post_clipping_min_integrated_intensity
        )
        ...
        post_filtered_mask = (
            cp.isin(modified_mask, post_filtered_labels) * modified_mask
        )

        if fill_holes:
            # print(f"Filling holes slice-wise in mask for {mask_filename}...")
            post_filtered_mask = fill_holes_3d_gpu(
                post_filtered_mask, min_hole_diameter
            )
            ...
            file_data.append(
                {
                    ...
                    "integrated_intensity": float(
                        post_integrated_intensity[post_labels == label][0]
                    ),
                    "mean_intensity": float(
                        post_mean_intensities[post_labels == label][0]
                    ),
                    "retained": 1,
                    "new_object_id": None,
                }
...
def process_directory(
    ...
    os.makedirs(output_dir, exist_ok=True)
    print(
        f"Starting clipping with raw_dir: {raw_dir}, mask_dir: {mask_dir}, output_dir: {output_dir}"
    )
    try:
        raw_files = [
            f for f in os.listdir(raw_dir) if f.lower().endswith((".tif", ".tiff"))
        ]
    except Exception as e:
    ...
    print(
        f"Found {len(raw_files)} raw files and {len(mask_files)} corresponding mask files."
    )
    all_data = []
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Files Completed"
        ):
            try:
...
def upsample_and_deblur_volume(
    volume,
    anisotropy,
    deblur_model_type="aniso_cyto2",
    diameter=30,
    z_axis=0,
    channels=[0, 0],
):
    ...
    vol_temp = transforms.resize_image(
        volume_proc.astype("float32").transpose(1, 0, 2),
        Ly=new_shape[0],
        Lx=new_shape[2],
        no_channels=True,
    ).transpose(1, 0, 2)
    print(f"[DEBUG] Shape after first resize: {vol_temp.shape}")
    vol_upsampled = transforms.resize_image(
        vol_temp, Ly=new_shape[1], Lx=new_shape[2], no_channels=True
    )
    ...
    img_iso1 = dn_model.eval(
        vol_upsampled.transpose(1, 0, 2), diameter=diameter, z_axis=0, channels=channels
    )
    img_iso1 = img_iso1.squeeze().transpose(1, 0, 2)
    img_iso2 = dn_model.eval(
        vol_upsampled.transpose(2, 0, 1), diameter=diameter, z_axis=0, channels=channels
    )
...
def segment_volume_from_array(
    ...
    if perform_anisotropic_deblur:
        ...
        vol_deblur = upsample_and_deblur_volume(
            vol_proc,
            anisotropy,
    ...
    try:
        ...
        if perform_anisotropic_deblur:
            if mask_final.ndim == 3:
                print(
                    f"[DEBUG] Resizing mask from shape {mask_final.shape} back to original z-dim {original_z}"
                )
                mask_final = resize(
                    mask_final,
...
def run_segmentation_from_memory(denoised_dict, output_path, workers, seg_params):
    ...
    with ThreadPoolExecutor(max_workers=workers) as executor:
        ...
        for image_name, (img_path, timepoint) in denoised_dict.items():
            futures.append(
                executor.submit(
                    ...
                    perform_anisotropic_deblur=seg_params.get(
                        "perform_anisotropic_deblur", False
                    ),
                    anisotropic_deblur_model=seg_params.get(
                        "anisotropic_deblur_model", "aniso_cyto2"
                    ),
        ...
        for future in tqdm(
            as_completed(futures), total=len(futures), desc="Segmenting images"
        ):
            try:
                future.result()
            except Exception as e:
...
def run_streamlit_app():
    ...
    st.write(
        "This application performs full workflow to process raw timelapse datasets using denoising, segmentation, and clipping to generate instance segmentation outputs.\nSupported formats: .zarr, .ome.zarr, .tif/.tiff, .czi."
    )

    global param_source, params_csv_dict, offset
    param_source = st.radio(
        "Parameter Source", options=["Manual", "CSV Import"], key="param_source"
    )
    params_csv_dict = {}
    if param_source == "CSV Import":
        param_file = st.file_uploader(
            "Upload Parameter CSV", type=["csv"], key="param_csv_file"
        )
    ...
    options = [
        ...
        "Denoising + Segmentation + Clipping",
    ]
    ...
    input_file_type = st.radio(
        "Input File Type",
        options=["Single S3 Path", "CSV File of S3 Paths"],
        index=0,
        key="input_file_type",
    )
    if input_file_type == "CSV File of S3 Paths":
        csv_input_file = st.file_uploader(
            "Upload CSV file containing S3 Paths", type=["csv"], key="input_csv"
        )
        ...
        input_path_value = st.text_input(
            "Input Path",
            value=get_param(
                "Input Path",
                "s3://allencell/aics/emt_timelapse_dataset/data/3500006256_11_raw_converted.ome.zarr",
            ),
            key="input_path",
        )
    ...
    output_path = st.text_input(
        "Output Path",
        value=get_param(
            "Output Path", "D:\\S3_Omezarr_DenoisePerTimepointScript1_Output"
        ),
        key="output_path",
    )
    timepoint_range = st.text_input(
        "Timepoint Range (e.g., 1-2 or leave blank for all)",
        value=get_param("Timepoint Range", "1-2"),
        key="timepoint_range",
    )

    with st.expander("Denoising Parameters", expanded=True):
        max_workers = st.number_input(
            "Max Workers (Denoising)",
            min_value=1,
            value=get_param("Max Workers (Denoising)", 4),
            step=1,
            key="denoise_max_workers",
        )
        file_workers = st.number_input(
            "File Workers (Denoising)",
            min_value=1,
            value=get_param("File Workers (Denoising)", 2),
            step=1,
            key="denoise_file_workers",
        )
        channel = st.number_input(
            "Channel (0=Brightfield, 1=EGFP, 2=CollagenIV Ab)",
            min_value=0,
            value=get_param("Channel", 1),
            step=1,
            key="denoise_channel",
        )
        offset = st.number_input(
            "Enter black reference pixel intensity offset for image subtraction",
            value=100,
            step=1,
            key="offset",
        )
        denoise_method = st.radio(
            "Denoising Method",
            options=["Dynamic Rescaling", "Constant Rescaling"],
            index=["Dynamic Rescaling", "Constant Rescaling"].index(
                get_param("Denoising Method", "Dynamic Rescaling")
            ),
            key="denoise_method",
        )
        save_raw = st.checkbox(
            "Save Raw Image", value=get_param("Save Raw Image", False), key="save_raw"
        )
    if denoise_method == "Constant Rescaling":
        with st.expander("Constant Rescaling Parameters", expanded=True):
            raw_max = st.number_input(
                "Raw Max", value=get_param("Raw Max", 2000), key="const_raw_max"
            )
            p99 = st.number_input(
                "99th Percentile",
                value=get_param("Percentile 99", 8000),
                key="const_p99",
            )
            denoised_min = st.number_input(
                "Denoised Min",
                value=get_param("Denoised Min", 0),
                key="const_denoised_min",
            )
            denoised_max = st.number_input(
                "Denoised Max",
                value=get_param("Denoised Max", 4),
                key="const_denoised_max",
            )
            const_params = {
                "raw_max": raw_max,
                "p99": p99,
    ...
    if param_source == "CSV Import":
        ...
        seg_workers = st.number_input(
            "Max Workers (Segmentation)",
            min_value=1,
            value=4,
            step=1,
            key="seg_max_workers",
    ...
    if process_mode in [
        ...
        with st.expander("Segmentation Parameters", expanded=True):
            ...
            seg_channels = [
                int(x.strip()) for x in seg_channels_str.split(",") if x.strip()
            ]
            seg_diameter = st.number_input(
                "Segmentation Diameter",
                min_value=1,
                value=get_param("Segmentation Diameter", 40),
                step=1,
                key="seg_diameter",
            )
            seg_flow_threshold = st.number_input(
                "Flow Threshold",
                value=get_param("Flow Threshold", 0.4),
                format="%.2f",
                key="seg_flow_threshold",
            )
            seg_stitch_threshold = st.number_input(
                "Stitch Threshold",
                value=get_param("Stitch Threshold", 0.5),
                format="%.2f",
                key="seg_stitch_threshold",
            )
            seg_cellprob_threshold = st.number_input(
                "Cell Probability Threshold",
                value=get_param("Cell Probability Threshold", -2.0),
                format="%.2f",
                key="seg_cellprob_threshold",
            )
            seg_do_3D = st.checkbox(
                "3D Processing",
                value=get_param("3D Processing", False),
                key="seg_do_3D",
            )
            seg_anisotropy = st.number_input(
                "Segmentation Anisotropy",
                value=get_param("Segmentation Anisotropy", 5.87),
                format="%.2f",
                key="seg_anisotropy",
            )
            seg_model_type = st.text_input(
                "Segmentation Model Type",
                value=get_param("Segmentation Model Type", "cyto3"),
                key="seg_model_type",
            )
            seg_pretrained_model = st.text_input(
                "Segmentation Pretrained Model",
                value=get_param(
                    "Segmentation Pretrained Model",
                    r"C:\scripts\BFS_segmentation_workflow\models\cellpose_1733942372.5497868",
                ),
                key="seg_pretrained_model",
            )
            perform_aniso = st.checkbox(
                "Perform anisotropic deblurring and upsampling",
                value=get_param("Perform anisotropic deblurring and upsampling", False),
                key="seg_perform_aniso",
            )
            aniso_deblur_model = st.selectbox(
                "Select anisotropic deblur model",
                options=["aniso_cyto2", "aniso_nuclei"],
                index=["aniso_cyto2", "aniso_nuclei"].index(
                    get_param("Select anisotropic deblur model", "aniso_cyto2")
                ),
                key="seg_aniso_deblur_model",
            )
            deblur_diam = st.number_input(
                "Deblur Diameter",
                value=get_param("Deblur Diameter", 30),
                step=1,
                key="seg_deblur_diam",
            )
            save_upsampled = st.checkbox(
                "Save deblurred/upsampled image",
                value=get_param("Save deblurred/upsampled image", False),
                key="seg_save_upsampled",
            )
            norm_params = get_param(
            ...
            seg_params = {
                ...
                "min_size": st.number_input(
                    "Min Size",
                    value=get_param("Segmentation Min Size", 15),
                    min_value=1,
                    step=1,
                    key="seg_min_size",
                ),
            }
            use_existing_denoised = st.checkbox(
                "Use existing denoised images for segmentation",
                value=get_param("Use Existing Denoised for Segmentation", False),
                key="use_existing_denoised",
            )
            use_existing_seg_masks = st.checkbox(
                "Use existing segmentation mask images for clipping",
                value=get_param("Use Existing Segmentation Masks for Clipping", False),
                key="use_existing_seg_masks",
            )
    else:
    ...
    if process_mode == "Denoising + Segmentation + Clipping":
        with st.expander("Clipping Parameters", expanded=True):
            clip_raw_dir = st.text_input(
                "Clipping Raw Directory",
                value=get_param(
                    "Clipping Raw Directory", str(Path(output_path) / "Denoised")
                ),
                help="Directory containing denoised images for clipping.",
                key="clip_raw_dir",
            )
            clip_mask_dir = st.text_input(
                "Clipping Mask Directory",
                value=get_param(
                    "Clipping Mask Directory",
                    str(Path(output_path) / "NucleiSegmentation"),
                ),
            ...
            clip_output_dir = st.text_input(
                "Clipping Output Directory",
                value=get_param(
                    "Clipping Output Directory", str(Path(output_path) / "ClippedMask")
                ),
                help="Directory to save clipped masks.",
                key="clip_output_dir",
            )
            clip_scaling_factor = st.number_input(
                "XY Clipping Percentile",
                value=get_param("Clipping XY Percentile", 55.0),
                format="%.2f",
                key="clip_xy",
            )
            clip_slice_percentile = st.number_input(
                "Z Slice Clipping Percentile",
                value=get_param("Clipping Z Percentile", 65.0),
                format="%.2f",
                key="clip_z",
            )
            clip_workers = st.number_input(
                "Clipping Workers",
                min_value=1,
                value=get_param("Clipping Workers", 2),
                step=1,
                key="clip_workers",
            )
            clip_relabel = st.checkbox(
                "Relabel after clipping",
                value=get_param("Clipping Relabel", False),
                key="clip_relabel",
            )
            clip_fill_holes = st.checkbox(
                "Fill holes",
                value=get_param("Clipping Fill Holes", False),
                key="clip_fill_holes",
            )
            clip_pre_min_size = st.number_input(
                "Pre-clipping Min Size",
                value=get_param("Clipping Pre-min Size", 0),
                min_value=0,
                step=1,
                key="clip_pre_min",
            )
            clip_post_min_size = st.number_input(
                "Post-clipping Min Size",
                value=get_param("Clipping Post-min Size", 0),
                min_value=0,
                step=1,
                key="clip_post_min",
            )
            clip_pre_min_mean = st.number_input(
                "Pre-clipping Min Mean Intensity",
                value=get_param("Clipping Pre-min Mean Intensity", 0.0),
                format="%.1f",
                key="clip_pre_mean",
            )
            clip_post_min_integrated_intensity = st.number_input(
                "Post-clipping Min Integrated Intensity",
                value=get_param("Clipping Post-min Integrated Intensity", 0.0),
                format="%.1f",
                key="clip_post_int",
            )
            clip_min_hole_diameter = st.number_input(
                "Clipping Min Hole Diameter",
                value=get_param("Clipping Min Hole Diameter", 64.0),
                format="%.1f",
                key="clip_min_diam",
            )
            skip_existing = st.checkbox(
                "Skip processing if output file exists",
                value=get_param("Skip Existing Clipping Files", False),
                key="clip_skip",
            )
            clip_params = {
                ...
                try:
                    ...
                    if use_existing_denoised:
                        ...
                        if denoise_method == "Dynamic Rescaling":
                            ...
                            denoised_dict = constant_denoise_directory(
                                input_path_value,
                                output_path,
                                {
                                    "gpu": True,
                    ...
                    if process_mode in [
                        ...
                        if use_existing_seg_masks:
                            st.info(
                                "Using existing segmentation mask images; skipping segmentation."
                            )
                        else:
                            run_segmentation_from_memory(
                                denoised_dict, output_path, int(seg_workers), seg_params
                            )

                    # ------------------ Step 3: Clipping ------------------
                    if process_mode == "Denoising + Segmentation + Clipping":
                        process_directory(
                            raw_dir=str(Path(output_path) / "Denoised"),
                            mask_dir=clip_params["mask_dir"],
                            output_dir=clip_params["output_dir"],
                            scaling_factor=clip_params["scaling_factor"],
                            slice_scaling_percentile=clip_params[
                                "slice_scaling_percentile"
                            ],
                            pre_clipping_min_size=clip_params["pre_clipping_min_size"],
                            post_clipping_min_size=clip_params[
                                "post_clipping_min_size"
                            ],
                            pre_clipping_min_mean_intensity=clip_params[
                                "pre_clipping_min_mean_intensity"
                            ],
                            post_clipping_min_integrated_intensity=clip_params[
                                "post_clipping_min_integrated_intensity"
                            ],
                        ...
                        params.update(
                            {
                                ...
                                "Segmentation Flow Threshold": seg_params[
                                    "flow_threshold"
                                ],
                                "Segmentation Stitch Threshold": seg_params[
                                    "stitch_threshold"
                                ],
                                "Segmentation CellProb Threshold": seg_params[
                                    "cellprob_threshold"
                                ],
                                ...
                                "Segmentation Pretrained Model": seg_params[
                                    "pretrained_model"
                                ],
                                "Normalization Parameters": seg_params["norm_params"],
                                "Perform Anisotropic Deblur": seg_params[
                                    "perform_anisotropic_deblur"
                                ],
                                "Anisotropic Deblur Model": seg_params[
                                    "anisotropic_deblur_model"
                                ],
                    ...
                    if process_mode == "Denoising + Segmentation + Clipping":
                        params.update(
                            {
                                "Clipping Raw Directory": str(
                                    Path(output_path) / "Denoised"
                                ),
                                ...
                                "Clipping Z Percentile": clip_params[
                                    "slice_scaling_percentile"
                                ],
                                "Clipping Workers": clip_params["workers"],
                                "Clipping Relabel": clip_params["relabel"],
                                "Clipping Fill Holes": clip_params["fill_holes"],
                                "Clipping Pre-min Size": clip_params[
                                    "pre_clipping_min_size"
                                ],
                                "Clipping Post-min Size": clip_params[
                                    "post_clipping_min_size"
                                ],
                                "Clipping Pre-min Mean Intensity": clip_params[
                                    "pre_clipping_min_mean_intensity"
                                ],
                                "Clipping Post-min Integrated Intensity": clip_params[
                                    "post_clipping_min_integrated_intensity"
                                ],
                                "Clipping Min Hole Diameter": clip_params[
                                    "min_hole_diameter"
                                ],
                                "Skip Existing Clipping Files": clip_params[
                                    "skip_existing"
                                ],
                            }
                        )
                    params_csv = (
                        Path(output_path) / f"processing_parameters_{timestamp}.csv"
                    )
    ...
    st.write(
        "Click the button below after processing to launch Napari and load all subfolders (e.g., Raw, Denoised, NucleiSegmentation, ClippedMask) as separate layers."
    )
    ...
    output_path_str = st.text_input(
        "Output Path for Napari", value=output_path, key="napari_output_path"
    )
...
def update_layers(output_path, position_id, viewer, generate_multiscale):
    ...
    for layer_name in target_layers:
        ...
        if data is None:
            print(
                f"No data found for layer '{layer_name}' at position '{position_id}'."
            )
            continue
        if data.ndim == 4:
            axes = ("Time", "Z", "Y", "X")
...
def napari_viewer_process(output_path):
        ...
        if layer_name.lower() in {"clippedmask", "nucleisegmentation"}:
            ...
            layer = viewer.add_labels(
                ...
                colormap=DirectLabelColormap(
                    colors=[
                        tuple(colormaps["turbo"](i)) for i in np.linspace(0, 1, 256)
                    ]
                ),
                axis_labels=axes,
                blending="additive",
            ...
            layer = viewer.add_image(
                ...
                contrast_limits=(
                    ...
                    if "raw" in layer_name.lower()
                        ...
                        [1500, 65535]
                        if "denoised" in layer_name.lower()
                        else [0, 65535]
                    )
                ),
                depiction="volume",
    ...
    positions = (
        get_unique_positions(denoised_dir) if os.path.exists(denoised_dir) else []
    )
    ...
    @magicgui(
        ...
        position={"widget_type": "ComboBox", "choices": positions},
    )
    ...
    def annotation_widget(
        ...
        if not object_id:
            ...
            return
        pos = (
            position_list.position.value
            if hasattr(position_list, "position")
            else "Unknown"
        )
    ...
    def save_annotations_button():
        if annotations:
            ...
            filename = os.path.join(
                output_path, f"annotations_{annotator}_{timestamp}.csv"
            )
            headers = [
                "Filename",
                "Object ID",
    ...
    for layer in viewer.layers:
        if isinstance(layer, Labels):
            layer.events.selected_label.connect(
                lambda event, layer=layer: update_object_id(layer)
            )
